...
namespace SMS.Data.BDS
{
    public class DAL_Build_Service
    {
        IFormatProvider iFormatProvider = CultureInfo.GetCultureInfo("en-GB");

        private string connection = "";
        public DAL_Build_Service(string ConnectionString)
        {
            connection = ConnectionString;
        }
        public DAL_Build_Service()
        {
       ...
       , string sortby, int? sortdirection, int? pagenumber, int? pagesize, string CompanyID, ref int isfetchcount)
        ...
        public int Upd_JiBeDeltaAutoSyncFlag_DL(int Vessel_ID, string SyncConn_String)
        {
            SqlParameter[] sqlprm = new SqlParameter[]
                                        { 
                                            new SqlParameter("@Vessel_ID",Vessel_ID)
                                        };
            return (int)SqlHelper.ExecuteNonQuery(SyncConn_String, CommandType.StoredProcedure, "UPD_Delta_Auto_Sync_Flag", sqlprm);
        }
        ...
        public int Upd_JibeDeltaData(int Vessel_ID, DateTime SetupCreationDate)
        {
            SqlParameter[] sqlprm = new SqlParameter[]
                                        { 
                                            new SqlParameter("@Vessel_ID",Vessel_ID),
                                            new SqlParameter("@Setup_Creation_Date",SetupCreationDate)
                                        };
             return Convert.ToInt16(SqlHelper.ExecuteScalar(connection, CommandType.StoredProcedure, "Get_Delta_DataLogTables_Data", sqlprm));
        }
        ...
        public DataTable GET_Delta_SQL_Script_Log_DL(int Vessel_ID,int SqlVersion)
        {
            SqlParameter[] sqlprm = new SqlParameter[]
                                        { 
                                        
                                             new SqlParameter("@Vessel_ID",Vessel_ID),
                                             new SqlParameter("@Last_Version",SqlVersion),                                       
                                        };
            return SqlHelper.ExecuteDataset(connection, CommandType.StoredProcedure, "GET_Delta_SQL_Script_Log",sqlprm).Tables[0];
        }
        ...
        // Description Method used to send script details to the JIT
        public DataTable GET_Delta_DLL_Log_DL(int Vessel_ID, int LastID)
        {
            SqlParameter[] sqlprm = new SqlParameter[]
                                        { 
                                        
                                             new SqlParameter("@Vessel_ID",Vessel_ID),
                                             new SqlParameter("@Last_ID",LastID),                                       
                                        };
            return SqlHelper.ExecuteDataset(connection, CommandType.StoredProcedure, "Get_All_Vessel_DLL_Log",sqlprm).Tables[0];
        }
