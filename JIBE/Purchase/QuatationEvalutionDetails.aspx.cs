...
public partial class Purchase_Quotation_Evaluation_Details_Gridview : System.Web.UI.Page
{
    ...
    DataTable selectedCols = new DataTable();
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        //string msg1 = String.Format("try{{calTotalFinalAmount();}}catch(exp){{}}");
        string msg1 = String.Format("try{{onIncludeVat('0');}}catch(exp){{}}");
    ...
    private void BindQuatationSendBySupplier()
    {
        ...
        try
        {
            ...
            using (BLL_PURC_Purchase objTechService = new BLL_PURC_Purchase())
            {
                ...
                selectedCols = dtsuppInfo.DefaultView.ToTable("Selected", false, "supplier", "short_name");
                ViewState["selectedCols"] = selectedCols;
    ...
    protected void btnRetrieve_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            ScriptManager.RegisterClientScriptBlock(this, GetType(), "none", "<script>onIncludeVat('1');</script>", false);
        }
        catch
        {
    ...
    private void BindGridOnRetriveButtonClick()
    {
        try
        {
            ...
            strTables.Append(@"  
                               ,isnull(vw.LAST_ORDER_AMOUNT,0) LAST_ORDER_AMOUNT from  (select * from  PURC_Dtl_Quoted_Prices where DOCUMENT_CODE ='" + Request.QueryString["Document_Code"].ToString() + @"')  a 
                                ...
                                left join PURC_ITEMS_LAST_ORDER_VIEW vw on M.item_ref_code=vw.item_ref_code
            ...
            foreach (GridViewRow dataItem in rgdSupplierInfo.Rows)
            {
                ...
                if (UDFLib.ConvertToDecimal(rgdSupplierInfo.DataKeys[dataItem.RowIndex].Values["ORDER_AMOUNT"].ToString()) < 1)
                {
                    ...
                    if ((chk.Checked))
                    {
                        ...
                        {
                            BackColor = "QtnEval-Det-ItemStyle-css";

                        }
                        else
                        {
                            BackColor = "QtnEval-Det-AltItemStyle-css";
                        }
                        suppCountBG++;
                        ...
                        str = @"(select supl.ITEM_REF_CODE
                                ...
                                ,PURC_Dtl_Quoted_Prices.QUOTATION_CODE
                                ,(((cast(QUOTED_RATE*" + exchange_rate + " as decimal(18,2))*supl.ORDER_QTY) - (cast(QUOTED_RATE*" + exchange_rate + " as decimal(18,2))*supl.ORDER_QTY*cast(isnull(PURC_DTL_REQSN.DISCOUNT,0) as decimal(18,2))/100) - (cast(QUOTED_RATE*" + exchange_rate + " as decimal(18,2))*supl.ORDER_QTY*cast(QUOTED_DISCOUNT as decimal(18,2))/100))) " + Col_supp_Alias + @"_Amount                                
                                ,(((cast(QUOTED_RATE*" + exchange_rate + " as decimal(18,2))*supl.ORDER_QTY) - (cast(QUOTED_RATE*" + exchange_rate + " as decimal(18,2))*supl.ORDER_QTY*cast(isnull(PURC_DTL_REQSN.DISCOUNT,0) as decimal(18,2))/100) - (cast(QUOTED_RATE*" + exchange_rate + " as decimal(18,2))*supl.ORDER_QTY*cast(QUOTED_DISCOUNT as decimal(18,2))/100)) * (ISNULL(PURC_Dtl_Quoted_Prices.VAT,0) / 100)) " + Col_supp_Alias + @"_Vat
                                ...
                                from PURC_Dtl_Quoted_Prices 
                                    ...
                                    inner join PURC_LIB_SYSTEM_PARAMETERS 
                                           on Code=Item_Type
                                    inner join PURC_DTL_REQSN on PURC_DTL_REQSN.DOCUMENT_CODE = PURC_Dtl_Quoted_Prices.DOCUMENT_CODE and PURC_DTL_REQSN.QUOTATION_CODE ='" + QUOTATION_CODE  + 
                                    "' where supplier_code='" + Col_supp_where
                            + "' and PURC_Dtl_Quoted_Prices.QUOTATION_CODE ='" + QUOTATION_CODE + "')  " + Col_supp_Alias + " on " + Col_supp_Alias + ".ITEM_REF_CODE = a.ITEM_REF_CODE ";
                        if (col_id == 18)
                        {
                        ...
                        strSql.Append(Environment.NewLine + " ," + Col_supp_Alias + "." + Col_supp_Alias + "_Vat ");
                        ...
                        //templateColumn.HeaderStyle.CssClass = " ";
                        ...
                        ////templateColumn.ItemStyle.HorizontalAlign = HorizontalAlign.Right;
                        ////templateColumn.ItemStyle.CssClass = BackColor;
                        //templateColumn.ItemStyle.CssClass = " ";
                        ////templateColumn.HeaderStyle.Width = 70;
                        ////templateColumn.ItemStyle.Width = 70;                        
                        //rgdQuatationInfo.Columns.Add(templateColumn);

                        templateColumn = new TemplateField();                        
                        templateColumn.HeaderTemplate = new DataGridTemplateLabel(ListItemType.Header, Col_supp, "", "");
                        templateColumn.HeaderStyle.CssClass = "hiderightborder";
                        //templateColumn.ItemTemplate = new DataGridTemplateLabel(ListItemType.Item, Col_supp + "_Amount", Col_supp, Col_supp_Alias + "_Amount");

                        templateColumn.ItemTemplate = new DataGridTemplateHidden(ListItemType.Item, Col_supp + "_Amount", Col_supp, Col_supp_Alias + "_Amount", Col_supp + "_UnitPrice", Col_supp_Alias + "_Rate", Col_supp + "_Vat", Col_supp_Alias + "_Vat");
                        //templateColumn.ItemTemplate = new DataGridTemplateHidden(ListItemType.Item, Col_supp + "_UnitPrice", "", Col_supp_Alias + "_Rate", Col_supp + "_Amount", Col_supp_Alias + "_Amount");

                        //templateColumn.ItemStyle.HorizontalAlign = HorizontalAlign.Right;                        
                        templateColumn.ItemStyle.CssClass = BackColor + " hiderightborder";
                        //templateColumn.HeaderStyle.Width = 70;
                        //templateColumn.ItemStyle.Width = 70;                        
                        rgdQuatationInfo.Columns.Add(templateColumn);
                        ...
                        templateColumn = new TemplateField();
                        templateColumn.HeaderTemplate = new DataGridTemplateLabel(ListItemType.Header, Col_supp, "", "");
                        templateColumn.HeaderStyle.CssClass = "hideleftborder";
                        ...
                        templateColumn.ItemStyle.CssClass = BackColor + " hideleftborder";
                        //templateColumn.HeaderStyle.Width = 70;
                        //templateColumn.ItemStyle.Width = 70;
                        rgdQuatationInfo.Columns.Add(templateColumn);
                        ...
                        templateColumnRemark.HeaderStyle.CssClass = "hiddencol";
                        ...
                        templateColumnRemark.ItemStyle.CssClass = BackColor + " hiddencol";
         ...
         a.active_status=1 order by M.ITEM_SERIAL_NO");
            ...
            foreach (string supp in arrSuppNameInShort)
            {
                ...
                if (supp != "" && supp != null)
                {
                    ...
                    info.AddMergedColumns(new int[] { column,column + 1, column + 2, column + 3, column + 4}, supp + "  (Port : " + ViewState["Portname"].ToString().Split(new char[] { ',' })[PortCount].ToString() + " ,Quoted Currency : " + ViewState["suppcurrency"].ToString().Split(new char[] { ',' })[PortCount].ToString() + ")");
                    column += 5;
                }
                PortCount++;
            }
    ...
    protected void rgdQuatationInfo_ItemDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        try
        {
            ...
            if (e.Row.RowType == DataControlRowType.Header)
            {
                ...
                //    s++;
                //    r = r + 4;
                //    if (e.Row.Cells[r].Text == "Remark")
                //    {
                //        e.Row.Cells[s].Style.Add("display", "none");
                //        e.Row.Cells[s + 1].Style.Add("display", "none");
                //        e.Row.Cells[s + 2].Style.Add("display", "none");
                //        e.Row.Cells[r + 1].ColumnSpan = 4;
                //        e.Row.Cells[r + 1].HorizontalAlign = HorizontalAlign.Right;
                //    }
                //     r = r + 3;
                // }
            }
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                Label lblCheapAmount = new Label();
                string cheapsupp = "";
                ...
                foreach (string supp in arrSupp)
                {
                    if (supp != "" && supp != null)
                    {
                        ...
                        Label lblAmount = ((Label)e.Row.FindControl(supp + "_Amount"));
                        if (Convert.ToDouble(lblAmount.Text) > 0)
                        {
                            if (!flag)
                            {
                                lblCheapAmount = lblAmount;
                                cheapsupp = supp;
                                flag = true;
                            }
                            else
                            {
                                if (Convert.ToDouble(lblAmount.Text) < Convert.ToDouble(lblCheapAmount.Text))
                                {
                                    lblCheapAmount = lblAmount;
                                    cheapsupp = supp;
                                }
                ...
                Label lbl = ((Label)e.Row.FindControl(cheapsupp + "_Amount"));
                if (Convert.ToDouble(lbl.Text) > 0)
                {
                    lbl.ForeColor = System.Drawing.Color.Green;
                    lbl.Font.Bold = true;

                    foreach (string objsupp in arrSupp)
                    {
                        if (objsupp != "" && objsupp != null)
                        {
                            Label lblAmount = ((Label)e.Row.FindControl(objsupp + "_Amount"));
                            if (Convert.ToDouble(lbl.Text) == Convert.ToDouble(lblAmount.Text))
                            {
    ...
    protected void btnExportToExcel_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            var grh = rgdQuatationInfo.HeaderRow;
            for (int i = 0; i < rgdQuatationInfo.HeaderRow.Cells.Count; i++)
            {
                string Suppname = grh.Cells[i].Text.ToLower();
                if (Suppname.Contains("remark"))
                {
                    //grh.Cells[i].Visible = false;
                    rgdQuatationInfo.Columns[i].Visible = false;
                }
            }
        ...
        try
        {
            ...
            foreach (GridViewRow item in rgdSupplierInfo.Rows)
            {
                ...
                if (chkQuaEvaluated.Checked && dicTotalAmount.ContainsKey(QuotationCode))
                {
                    ...
                    {
                        //btnRequestAmount.Visible = true;
                        String msg1 = String.Format("alert('Total Approval amount is greater than Budget limit,Please request for increase Budget limit.');");
                        ScriptManager.RegisterStartupScript(Page, Page.GetType(), "msg", msg1, true);
                        String msgmodal = String.Format("showModal('divApprove');");
                        ScriptManager.RegisterStartupScript(Page, Page.GetType(), "msgonFinalmodal", msgmodal, true);
                    }
    ...
    private void RenderHeader(HtmlTextWriter output, Control container)
    {
        ...
        for (int i = 0; i < container.Controls.Count; i++)
        {
            ...
            if (!info.MergedColumns.Contains(i))
            {
                ...
                if (info.StartColumns.Contains(i))
                {
                    if (suppCount % 2 == 0)
                    {
                        HeadCss = "QtnEval-Det-HeaderStyle-css";
                    }
                    else
                    {
                        HeadCss = "QtnEval-Det-HeaderStyle-css";
                    ...
                    output.Write(string.Format("<th align='center' class='" + HeadCss + "' colspan='{0}'>{1}</th>",
                             info.StartColumns[i], info.Titles[i], "Supp***"));
                    suppCount++;
        ...
        for (int i = 0; i < info.MergedColumns.Count; i++)
        {
            ...
            {
                //cell.CssClass = "QtnEval-HeaderStyle-css";  Commented & add By Rupsing
                if (cell.CssClass != "")
                    cell.CssClass = cell.CssClass + " QtnEval-Det-HeaderStyle-css";
                else
                    cell.CssClass = "QtnEval-Det-HeaderStyle-css";
            ...
            if (Alternategroup)
            {
                //cell.CssClass = "QtnEval-AltHeaderStyle-css";  Commented & add By Rupsing
                if (cell.CssClass != "")
                    cell.CssClass = cell.CssClass + " QtnEval-Det-HeaderStyle-css";
                else
                    cell.CssClass = "QtnEval-Det-HeaderStyle-css";
    ...
    private class MergedColumnsInfo
    {
        ...
        public void AddMergedColumns(int[] columnsIndexes, string title)
        {
            ...
            StartColumns.Add(columnsIndexes[0], columnsIndexes.Length - 1);
            Titles.Add(columnsIndexes[0], title.Split('(')[0].Trim());
        }
    }
    ...
    protected void rgdSupplierInfo_ItemDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
          ...
          //((CheckBox)item.FindControl("chkQuaEvaluated")).Attributes.Add("onclick", "javascript:GetSupplierStatus('" + rgdSupplierInfo.DataKeys[item.RowIndex].Values["SUPPLIER"].ToString() + "')");
            ...
            HiddenField hdnsupplier_code = (HiddenField)e.Row.FindControl("hdfSupplier_code");
            string strTemp = hdnsupplier_code.Value;
            ...
            {
                ((HyperLink)item.FindControl("ImgAttachment")).Visible = true;
            }
            else
            {
                ((HyperLink)item.FindControl("ImgAttachment")).Visible = false;
            }
    ...
    protected class DataGridTemplateImage : System.Web.UI.Page, ITemplate
    {
        ...
        protected void img_DataBinding(object s, EventArgs e)
        {
            ...
            string suppCode = DataFieldName.ToUpper().Replace("SUPP", "");
            suppCode = suppCode.Split('_')[0];
            suppCode = suppCode.Split('_')[0].Substring(0, suppCode.Length - 1);
            if (strremark != "")
            {
                img.Attributes.Add("Title", "cssbody=[dvbdy1] cssheader=[dvhdr1] header=[] body=[" + strremark + "]");
                img.AlternateText = suppCode + " : " + strremark;
            }
            else
            {
    ...
    protected class DataGridTemplateLabel : System.Web.UI.Page, ITemplate
    {
        ...
        public void InstantiateIn(System.Web.UI.Control container)
        {
            ...
            //if (columnName.Contains("_Amount"))
            //{
            //    Label txtSlash = new Label();
            //    txtSlash.Text = "/";
            //    container.Controls.Add(txtSlash);
            //}
    ...
    protected class DataGridTemplateHidden : System.Web.UI.Page, ITemplate
    {
        ListItemType templateType;
        string columnName;
        string columnName1;
        ...
        public DataGridTemplateHidden(ListItemType type, string colname, string HeaderText, string DataField, string colname1, string DataField1, string colname2, string DataField2)
        {
            templateType = type;
            columnName = colname;
            ...
            Header = HeaderText;
            DataFieldName = DataField;
            DataFieldName1 = DataField1;
            DataFieldName2 = DataField2;
        ...
        public void InstantiateIn(System.Web.UI.Control container)
        {
            Label lb_control = new Label();
            HiddenField hdf_control = new HiddenField();
            HiddenField hdf_control2 = new HiddenField();

            switch (templateType)
            {
                case ListItemType.Item:

                    lb_control.ID = columnName;
                    container.Controls.Add(lb_control);
                    lb_control.DataBinding += new EventHandler(lb_DataBinding);
                    container.Controls.Add(lb_control);

                    hdf_control.ID = columnName1;
                    container.Controls.Add(hdf_control);
                    hdf_control.DataBinding += new EventHandler(Hidden_DataBinding);

                    hdf_control2.ID = columnName2;
                    container.Controls.Add(hdf_control2);
                    hdf_control2.DataBinding += new EventHandler(Hidden_DataBinding2);
                    break;
        ...
        void Hidden_DataBinding(object sender, EventArgs e)
        {
            ...
            string strvalue = ((DataRowView)container.DataItem)[DataFieldName1].ToString();


            if (decimal.TryParse(strvalue, out outRes))
            {
                hdn.Value = outRes.ToString("0.##");
            }
            else
            {
                hdn.Value = ((DataRowView)container.DataItem)[DataFieldName1].ToString();
            }             
        }

        void Hidden_DataBinding2(object sender, EventArgs e)
        {
            HiddenField hdn = (HiddenField)sender;
            GridViewRow container = (GridViewRow)hdn.NamingContainer;
            ...
            string strvalue = ((DataRowView)container.DataItem)[DataFieldName2].ToString();
            ...
            {
                hdn.Value = ((DataRowView)container.DataItem)[DataFieldName2].ToString();
            }
        }
    ...
    protected void rgdQuatationInfo_PreRender()
    {
        ...
        foreach (GridViewRow Item in rgdQuatationInfo.Rows)
        {
            Table table = rgdQuatationInfo.Rows[0].Parent as Table;
            int Count = 0;
            ...
            foreach (string supp in arrSupp)
            {
                if (supp != "" && supp != null)
                {
                    Image img = ((Image)Item.FindControl(supp + "_img"));

                    if (img != null)
                    {
                        if (img.AlternateText != "")
                        {
                            Remarks = Remarks + "," + img.AlternateText;
                        }
                    }
                }
            }
            ...
            if (Remarks.Trim(',') != "")
            {
                arr.Add(Item.RowIndex + AddRow);
                AddRow++;

                for (int J = 0; J <= arr.Count - 1; J++)
                {
                    GridViewRow row = new GridViewRow(-1, -1, DataControlRowType.DataRow, DataControlRowState.Normal);
                    row.Cells.Add(CreateColumn(Remarks.Trim(',')));
                    table.Rows.AddAt(Convert.ToInt32(arr[J]) + Count, row);
                    Count = Count + 1;
                }
            }
        }
    ...
    private TableCell CreateColumn(string Remarks)
    {
        DataTable dt = ((DataTable)ViewState["selectedCols"]);
        foreach (DataRow row in dt.Rows)
        {
            Remarks = Remarks.Replace(row["supplier"].ToString(), row["short_name"].ToString());
        }
        //dv.RowFilter = "supplier='" + Remarks.Split(':')[0].Trim() + "'";        
        //string suppShort =  dv.ToTable().Rows[0]["short_name"].ToString();
        //Remarks.Replace(Remarks.Split(':')[0].Trim(), suppShort);
        Remarks = Remarks.Replace(",", "<br />");
        TableCell cell = new TableCell();
        cell.ColumnSpan = rgdQuatationInfo.Columns.Count;
        cell.BackColor = System.Drawing.Color.White;
        cell.ForeColor = System.Drawing.Color.Black;
        cell.Attributes.Add("class", "float-left");
        cell.Controls.Add(new LiteralControl(Remarks));
        return cell;
