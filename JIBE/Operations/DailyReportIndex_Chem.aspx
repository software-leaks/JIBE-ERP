<%@ Page Title="Daily Report Index Chemical" Language="C#" MasterPageFile="~/Site.master"
    AutoEventWireup="true" CodeFile="DailyReportIndex_Chem.aspx.cs" Inherits="Operations_DailyReportIndex_Chem" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="HeadContent" runat="Server">
    ...
    <script type="text/javascript">
        function showdetails(querystring) {
            ...
            if (query[2] == 'A')
                window.open("Arrival_Report_Chem.aspx?TelegramID=" + query[0] + "&VesselID=" + query[1] + "&ReportType=" + query[2] + "&filters=" + query[3]);
            if (query[2] == 'X')
                window.open("NoonReport_Port_Chem.aspx?TelegramID=" + query[0] + "&VesselID=" + query[1] + "&ReportType=" + query[2] + "&filters=" + query[3]);
            if (query[2] == 'N')
                window.open("NoonReport_Sea_Chem.aspx?TelegramID=" + query[0] + "&VesselID=" + query[1] + "&ReportType=" + query[2] + "&filters=" + query[3]);
            if (query[2] == 'D')
                window.open("Departure_Report_Chem.aspx?TelegramID=" + query[0] + "&VesselID=" + query[1] + "&ReportType=" + query[2] + "&filters=" + query[3]);
                                ...
                                <asp:TemplateField>
                                    <ItemTemplate>
                                        <asp:Image ID="imgRecordInfo" ImageAlign="AbsMiddle" ImageUrl="~/Images/RecordInformation.png"
                                            Height="16px" Width="16px" runat="server" onclick='<%# "Get_Record_Information(&#39;OPS_Telegrams&#39;,&#39; TELEGRAM_ID="+Eval("TELEGRAM_ID").ToString()+" and Vessel_ID="+Eval("Vessel_ID").ToString()+"&#39;,event,this)" %>'
                                            AlternateText="info" />
                                    </ItemTemplate>
                                </asp:TemplateField>
                            </Columns>
