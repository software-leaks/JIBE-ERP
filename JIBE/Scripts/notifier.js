    ...
    $.extend(
		{
		    notifier: {
		        ...
		        css: function (notice) {
		            ...
		            for (var n = 0; n < skin.length; n++) { if (n < 4) { css += this.skins[skin[n]].replace(/#_/g, "#" + this.defaults.core).replace(/#-/g, "#" + notice.id); } }
		            
		            try {
		                if (skin.length > 5) {
		                    css = css.replace(/_&position/gi, skin[4]);
		                    css = css.replace(/_&top/gi, skin[5] + 'px');
		                    css = css.replace(/_&right/gi, skin[6] + 'px');
		                }


		                else {
		                    css = css.replace(/_&position/gi, 'fixed');
		                    css = css.replace(/_&top/gi, '18%');
		                    css = css.replace(/_&right/gi, '50%')
		                }
