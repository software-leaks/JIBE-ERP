...
//        rate_obj.innerHTML = obj.value.split(",")[0];
        rate_obj.value = obj.value.split(",")[0];
...
//        var txtDiscount = document.getElementById(prefix_id + "_" + arrquotation_codes_compare[j].toString() + "_Discount").innerHTML;
    //    var txtRate = document.getElementById(prefix_id + "_" + arrquotation_codes_compare[j].toString() + "_UnitPrice").innerHTML;
        var txtRate = document.getElementById(prefix_id + "_" + arrquotation_codes_compare[j].toString() + "_UnitPrice").value;
        var iQty = (ordqty_val.strim() == "") ? 0 : ordqty_val;
        var iDiscount = 0; // (txtDiscount.strim() == "") ? 0 : txtDiscount;
...
//debugger;
    ...
    for (var i = 1; i <= griditems.rows.length - 1; i++) {
        ...
        for (var j = 0; j < arrquotation_codes_compare.length; j++) {
            if (checked == true) {
                ...
                if (arrquotation_codes_compare[j].toString() != ckk_id_postfix) {
                    ...
                    if (document.getElementById(chk_id_other_local) != null) {
        ...
        if (document.getElementById(chk_id) != null) 
        {
            if (document.getElementById(chk_id).disabled != true) {
            if (i != 1) {
                 if (parseFloat(document.getElementById(UnitPrice_id).value) > 0)
...
//Include / Exclude vat calculation
//Vat Changes
function onIncludeVat(vatCalc) {
    //debugger;
    //alert(vatCalc);
    if (vatCalc == "1") {
        _vatAdded = false;
        _vatAddedOther = false;
    }

    //alert(_vatAdded);

    var chkIncVat = document.getElementById("ctl00_MainContent_chkIncludeVat");
     
    var griditems = document.getElementById("ctl00_MainContent_rgdQuatationInfo");
    var suppliergrid = document.getElementById("ctl00_MainContent_rgdSupplierInfo");
    var quotation_codes_compare = "";

    if (document.getElementById("ctl00_MainContent_hdfquotation_codes_compare") != null) {
        quotation_codes_compare = document.getElementById("ctl00_MainContent_hdfquotation_codes_compare").value; ;
    }

    //alert(griditems);

    var arrquotation_codes_compare = quotation_codes_compare.split(",");
    var _chkVatAdded = _vatAdded;
    ...
    if (griditems != null) {
        for (var i = 0; i < arrquotation_codes_compare.length; i++) {
            for (var j = 2; j < griditems.rows.length; j++) {
                ...
                if (j < 10) {
                    ...
                    vat_id = "ctl00_MainContent_rgdQuatationInfo_ctl0" + j.toString() + "_" + arrquotation_codes_compare[i].toString() + "_Vat";
                }
                else {
                    amt_id = "ctl00_MainContent_rgdQuatationInfo_ctl" + j.toString() + "_" + arrquotation_codes_compare[i].toString() + "_Amount";
                    vat_id = "ctl00_MainContent_rgdQuatationInfo_ctl" + j.toString() + "_" + arrquotation_codes_compare[i].toString() + "_Vat";
                }

                if (document.getElementById(amt_id) != null) {
                    //alert(parseFloat(document.getElementById(amt_id).innerHTML));

                    if (document.getElementById(vat_id) != null) {
                        //alert(parseFloat(document.getElementById(vat_id).value));
                        vatAmt = 0;
                        //vatAmt = parseFloat(document.getElementById(amt_id).innerHTML) * parseFloat(document.getElementById(vat_id).value) / 100;
                        vatAmt = parseFloat(document.getElementById(vat_id).value);
                        //alert(vatAmt);
                        if (chkIncVat.checked != true) {
                            if (_vatAdded == true) {
                                document.getElementById(amt_id).innerHTML = (parseFloat(document.getElementById(amt_id).innerHTML) - parseFloat(vatAmt)).toFixed(2);
                                _chkVatAdded = false;
                            }
                        }
                        else {
        ...
        _vatAdded = _chkVatAdded;

        calculate();
    }
}

//function onIncludeVat() {    
...
//        var lbltxtVat = (k < 10) ? "ctl00_MainContent_rgdSupplierInfo_ctl0" + k.toString() + "_txtVat" : "ctl00_MainContent_rgdSupplierInfo_ctl" + k.toString() + "_txtVat";

//        if (document.getElementById(lbltxtGrandTotal_id) != null || document.getElementById(lbltxtGrandTotal_id) != undefined) {
//            net_Amount = parseFloat(document.getElementById(lbltxtGrandTotal_id).innerHTML);
//            //Include / Exclude vat Calculation
//            if (document.getElementById(lbltxtVat) != null || document.getElementById(lbltxtVat) != undefined) {
//                vat = parseFloat(document.getElementById(lbltxtVat).innerHTML);
//                if (chkIncVat.checked != true) {
//                    net_Amount = net_Amount - vat;
//                }
//                else {
//                    net_Amount = net_Amount + vat;
//                }
//                document.getElementById(lbltxtGrandTotal_id).innerHTML = net_Amount.toFixed(2);
//            }
//            Tot_Net_Amount = Tot_Net_Amount + net_Amount;
//        }
//    }

//    document.getElementById('ctl00_MainContent_rgdSupplierInfo_ctl01_lblTotNetAmount').innerHTML = "(" + Tot_Net_Amount.toFixed(2) + ")";  
//}

function calTotalFinalAmount() {
    //debugger;
    var Tot_Net_Amount = 0; 
    var net_Amount = 0;
    var vatPer = 0;
    ...
    var _chkvatAddedOther = _vatAddedOther;
...
//        }
        ...
        var SuppTotAmt = (k < 10) ? "ctl00_MainContent_rgdSupplierInfo_ctl0" + k.toString() + "_lblSuppTotAmt" : "ctl00_MainContent_rgdSupplierInfo_ctl" + k.toString() + "_lblSuppTotAmt";


        var PkgHaldOld_id = (k < 10) ? "ctl00_MainContent_rgdSupplierInfo_ctl0" + k.toString() + "_lblPkgOld" : "ctl00_MainContent_rgdSupplierInfo_ctl" + k.toString() + "_lblPkgOld";
        var freightCostOld_id = (k < 10) ? "ctl00_MainContent_rgdSupplierInfo_ctl0" + k.toString() + "_lblFreight_CostOld" : "ctl00_MainContent_rgdSupplierInfo_ctl" + k.toString() + "_lblFreight_CostOld";
        var OthCostOld_id = (k < 10) ? "ctl00_MainContent_rgdSupplierInfo_ctl0" + k.toString() + "_lblOtherCostOld" : "ctl00_MainContent_rgdSupplierInfo_ctl" + k.toString() + "_lblOtherCostOld";
        var TruckCostOld_id = (k < 10) ? "ctl00_MainContent_rgdSupplierInfo_ctl0" + k.toString() + "_lblTruck_CostOld" : "ctl00_MainContent_rgdSupplierInfo_ctl" + k.toString() + "_lblTruck_CostOld";
        var bargeCostOld_id = (k < 10) ? "ctl00_MainContent_rgdSupplierInfo_ctl0" + k.toString() + "_lblBarge_Workboat_CostOld" : "ctl00_MainContent_rgdSupplierInfo_ctl" + k.toString() + "_lblBarge_Workboat_CostOld";

        var SuppTotAmtOld = (k < 10) ? "ctl00_MainContent_rgdSupplierInfo_ctl0" + k.toString() + "_lblSuppTotAmtOld" : "ctl00_MainContent_rgdSupplierInfo_ctl" + k.toString() + "_lblSuppTotAmtOld";
        ...
        if (document.getElementById(vat_id) != null || document.getElementById(vat_id) != undefined) {
            vatPer = parseFloat(document.getElementById(vat_id).value);
                        
            if (chkIncVat.checked != true) {
                document.getElementById(lblvat_id).innerHTML = parseFloat(0).toFixed(2);                

                if (_vatAddedOther == true) {
                    document.getElementById(SuppTotAmt).innerHTML = (parseFloat(document.getElementById(SuppTotAmt).innerHTML) - (parseFloat(document.getElementById(SuppTotAmtOld).value) * parseFloat(vatPer) / 100)).toFixed(2);

                    document.getElementById(PkgHald_id).innerHTML = (parseFloat(document.getElementById(PkgHald_id).innerHTML) - (parseFloat(document.getElementById(PkgHaldOld_id).value) * parseFloat(vatPer) / 100)).toFixed(2);
                    document.getElementById(freightCost_id).innerHTML = (parseFloat(document.getElementById(freightCost_id).innerHTML) - (parseFloat(document.getElementById(freightCostOld_id).value) * parseFloat(vatPer) / 100)).toFixed(2);
                    document.getElementById(OthCost_id).innerHTML = (parseFloat(document.getElementById(OthCost_id).innerHTML) - (parseFloat(document.getElementById(OthCostOld_id).value) * parseFloat(vatPer) / 100)).toFixed(2);
                    document.getElementById(TruckCost_id).innerHTML = (parseFloat(document.getElementById(TruckCost_id).innerHTML) - (parseFloat(document.getElementById(TruckCostOld_id).value) * parseFloat(vatPer) / 100)).toFixed(2);
                    document.getElementById(bargeCost_id).innerHTML = (parseFloat(document.getElementById(bargeCost_id).innerHTML) - (parseFloat(document.getElementById(bargeCostOld_id).value) * parseFloat(vatPer) / 100)).toFixed(2);                    

                    _chkvatAddedOther = false;
                }
            }
            else {
                document.getElementById(lblvat_id).innerHTML = vatPer.toFixed(2);

                if (_vatAddedOther != true) {
                    document.getElementById(SuppTotAmt).innerHTML = (parseFloat(document.getElementById(SuppTotAmt).innerHTML) + (parseFloat(document.getElementById(SuppTotAmt).innerHTML) * parseFloat(vatPer) / 100)).toFixed(2);

                    document.getElementById(PkgHald_id).innerHTML = (parseFloat(document.getElementById(PkgHald_id).innerHTML) + (parseFloat(document.getElementById(PkgHald_id).innerHTML) * parseFloat(vatPer) / 100)).toFixed(2);
                    document.getElementById(freightCost_id).innerHTML = (parseFloat(document.getElementById(freightCost_id).innerHTML) + (parseFloat(document.getElementById(freightCost_id).innerHTML) * parseFloat(vatPer) / 100)).toFixed(2);
                    document.getElementById(OthCost_id).innerHTML = (parseFloat(document.getElementById(OthCost_id).innerHTML) + (parseFloat(document.getElementById(OthCost_id).innerHTML) * parseFloat(vatPer) / 100)).toFixed(2);
                    document.getElementById(TruckCost_id).innerHTML = (parseFloat(document.getElementById(TruckCost_id).innerHTML) + (parseFloat(document.getElementById(TruckCost_id).innerHTML) * parseFloat(vatPer) / 100)).toFixed(2);
                    document.getElementById(bargeCost_id).innerHTML = (parseFloat(document.getElementById(bargeCost_id).innerHTML) + (parseFloat(document.getElementById(bargeCost_id).innerHTML) * parseFloat(vatPer) / 100)).toFixed(2);
                    
                    _chkvatAddedOther = true;
                }
            }
        }        

        Tot_Amount = document.getElementById(lblTot_Amount_id).innerHTML;
        ...
        net_TotalAmount = (parseFloat(Tot_Amount) + parseFloat(Surcharge) + parseFloat(PkgHald) + parseFloat(freightCost) + parseFloat(OthCost) + parseFloat(TruckCost) + parseFloat(bargeCost)).toFixed(2);

        if (parseFloat(Tot_Amount) > 0) {
            document.getElementById(lbltxtGrandTotal_id).innerHTML = parseFloat(net_TotalAmount).toFixed(2);
        }
    ...
    _vatAddedOther = _chkvatAddedOther;
}
        ...
        if (Tot_Amount != 0) {
            ...
            for (var k = 2; k <= suppliergrid.rows.length; k++) {
                ...
                if (chk_all_ColumnID.search(Quotation_code_suppgrid_val) != -1) {
                    ...
                    //Vat Changes
                    //net_Amount = (Tot_Amount - Discount + Surcharge + parseFloat(PkgHald) + parseFloat(freightCost) + parseFloat(OthCost) + parseFloat(TruckCost) + parseFloat(bargeCost)).toFixed(2);
                    net_Amount = (Tot_Amount + Surcharge + parseFloat(PkgHald) + parseFloat(freightCost) + parseFloat(OthCost) + parseFloat(TruckCost) + parseFloat(bargeCost)).toFixed(2);
...
function UpdateEvalution() {
    ...
    for (var i = 0; i < arrquotation_codes_compare.length; i++) {
        ...
        for (var j = 2; j < griditems.rows.length; j++) {
            ...
            var itemNull;
            if (j < 10) {                
                itemNull = document.getElementById("ctl00_MainContent_rgdQuatationInfo_ctl0" + j.toString() + "_hdfItemRef_Code");            
            }
            else {                
                itemNull = document.getElementById("ctl00_MainContent_rgdQuatationInfo_ctl" + j.toString() + "_hdfItemRef_Code");                
            }

            if (itemNull != null || itemNull != undefined)
            {
            if (j < 10) {
            ...
            if (chk_id_obj.checked == true && chk_id_obj.disabled != true) {
                ...
                var strtxtOunit = 0;
                if (document.getElementById(ordunit_id) != null) {
                    strtxtOunit = document.getElementById(ordunit_id).innerHTML;
                }
                var strRate = 0;
                var strItemType = 0;

                strRate = document.getElementById(Rate_id).value;
                ...
                if (parseFloat(strRate) != 0) {
                    ...
                    strQuery = strQuery + "  Update dbo.PURC_Dtl_Supply_Items Set Modified_By=" + userID + ", Date_Of_Modification=getdate(),Order_QTY='" + strtxtORqty + "' ,SYNC_FLAG='0' " + "where ITEM_REF_CODE ='" + itemrefcode_val + "' and DOCUMENT_CODE='" + DocumentCode + "' "
                }
        ...
        case "1":
            ...
            for (var i = 2; i < griditems.rows.length; i++) {
                ...
                for (var j = 0; j < arrquotation_codes_compare.length; j++) {
                    ...
                    if (document.getElementById(UnitPrice_id) != null) {
                        if (parseFloat(document.getElementById(UnitPrice_id).value) > 0) {
...
function asyncGet_Purc_Supplier_Attachments(objthis,evt,Requisition_code, Vessel_ID, SUPPLIER_CODE) {
//    debugger;
    var lastExecutorPB;
//    var objthis;   
    ...
    var service = Sys.Net.WebServiceProxy.invoke('../JibeWebService.asmx', 'async_Purc_Supplier_Attachments', false, { "Reqsnno": Requisition_code, "vesselID": Vessel_ID, "SuppCode": SUPPLIER_CODE }, onSuccess_asyncGet_Purc_Supplier_Attachments, Onfail, new Array(evt, objthis));    
    lastExecutorPB = service.get_executor();

}

function onSuccess_asyncGet_Purc_Supplier_Attachments(retVal, Args) {     
    js_ShowToolTip_Fixed(retVal, Args[0], Args[1], "Attachment");
}
